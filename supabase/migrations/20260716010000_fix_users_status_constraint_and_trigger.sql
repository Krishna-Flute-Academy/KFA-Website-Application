-- Update public.handle_new_user() trigger function to default new signups to 'pending' role and 'active' status
-- This complies with the legacy 'users_status_check' constraint requiring status to be either 'active' or 'inactive'.
...
BEGIN
  ...
  IF existing_id IS NOT NULL AND existing_id != NEW.id THEN
    ...
    SELECT NEW.id, 
           ...
           COALESCE(NEW.raw_user_meta_data->>'role', role), 
           status, 
           join_date, 
           teacher_id, 
           level
  ...
  ELSE
    ...
    VALUES (
      ...
      COALESCE(NEW.raw_user_meta_data->>'role', 'pending'), -- DEFAULT TO PENDING ROLE
      'active', -- DEFAULT TO ACTIVE STATUS (Matches users_status_check constraint)
      CURRENT_DATE
    )
    ON CONFLICT (id) DO NOTHING;
  END IF;
