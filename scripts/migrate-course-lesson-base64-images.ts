...
interface LessonMigrationRecord {
    id: string;
    lesson_number: number;
    title: string;
    originalDescription: string;
    originalChars: number;
    images: Base64ImageMatch[];
}
...
function parseBase64Images(lessonId: string, lessonTitle: string, html: string): Base64ImageMatch[] {
...
async function runMigration() {
    ...
    const targetLessons: LessonMigrationRecord[] = [];
    ...
    for (let i = 0; i < targetLessons.length; i++) {
        ...
        const uploadResults: { originalDataUrl: string; publicUrl: string }[] = [];
        ...
        for (let j = 0; j < lesson.images.length; j++) {
            ...
            if (uploadErr) {
                ...
                if (uploadErr.message?.includes('already exists') || uploadErr.message?.includes('Duplicate') || (uploadErr as any)?.statusCode === '409') {
