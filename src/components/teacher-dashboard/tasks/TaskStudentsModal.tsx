...
import { deriveStudentAssignmentStatus } from '../../../lib/assignment-service';
                ...
                <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/40 rounded-t-3xl shrink-0">
                    <div className="min-w-0 flex-1 pr-3">
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {batch.targetType === 'individual' ? (
                                <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-amber-100 text-amber-900 border border-amber-300 dark:bg-amber-950/60 dark:text-amber-300 shrink-0">
                                    👤 Individual
                                </span>
                            ) : (
                                <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-indigo-100 text-indigo-900 border border-indigo-300 dark:bg-indigo-950/60 dark:text-indigo-300 shrink-0">
                                    👥 For Everyone
                                </span>
                            )}
                        </div>
                ...
                <div className="p-4 overflow-y-auto flex-1 divide-y divide-slate-100 dark:divide-slate-800 space-y-1">
                    {filteredSubmissions.length > 0 ? (
                        filteredSubmissions.map(sub => {
                            const derived = deriveStudentAssignmentStatus(sub.status, batch.dueDate);
                                ...
                                <div key={sub.id} className="pt-2 pb-2 flex items-center justify-between gap-3">
                                    <div className="flex items-center gap-3 min-w-0 flex-1">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <span className="text-[10px] text-slate-400 block truncate">
                                                {sub.status === 'submitted' ? `Submitted ${new Date(sub.submitted_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })} · Awaiting Review` :
                                                 sub.status === 'approved' ? `Approved (Score: ${sub.score ?? '—'}/10)` :
                                                 sub.status === 'reviewed' ? `Needs Revision` :
                                                 derived.isPastDue ? 'Not Submitted · Past Due' : 'Not Submitted · Pending'}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase border ${
                                            ...
                                            derived.isPastDue ? 'bg-rose-100 text-rose-900 border-rose-300 dark:bg-rose-950/60 dark:text-rose-300' :
                                            'bg-slate-100 text-slate-500 border-slate-200 dark:bg-slate-800 dark:text-slate-400'
                                        }`}>
                                            {sub.status === 'submitted' ? 'Awaiting Review' :
                                             sub.status === 'reviewed' ? 'Needs Revision' :
                                             sub.status === 'approved' ? 'Approved' :
                                             derived.isPastDue ? 'Past Due' : 'Pending'}
                                        </span>
